...
inline int CellMap::isAlive(size_t c, size_t l)
{
...
void CellMap::updateNeighbourCount()
{
	...
	for (size_t l = 0; l < h; ++l)
	{
		for (size_t c = 0; c < w; ++c)
		{
			...
			{
				if (c > 0 && l > 0)
					*(cell_ptr - w - 1) += NEIGHBOR_COUNT; // top left
				if (l > 0)
					*(cell_ptr - w) += NEIGHBOR_COUNT; // top
				if (c < w - 1 && l > 0)
					*(cell_ptr - w + 1) += NEIGHBOR_COUNT; // top right
				if (c > 0)
					*(cell_ptr - 1) += NEIGHBOR_COUNT; // left
				if (c < w - 1)
					*(cell_ptr + 1) += NEIGHBOR_COUNT; // right
				if (c > 0 && l < h - 1)
					*(cell_ptr + w - 1) += NEIGHBOR_COUNT; // below left
				if (l < h - 1)
					*(cell_ptr + w) += NEIGHBOR_COUNT; // below
				if (c < w - 1 && l < h - 1)
					*(cell_ptr + w + 1) += NEIGHBOR_COUNT; // below right
			}
			++cell_ptr;
		}
	}
