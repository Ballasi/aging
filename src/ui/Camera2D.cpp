...
Rect Camera2D::get_view_bounds(float aspect_ratio, Universe *universe) {
    ...
    Coord top_left = Coord(BigInt((int) std::ceil(rectf.topLeft().x()) - 1), BigInt((int) std::ceil(rectf.bottomRight().y())));
    Coord bottom_right = Coord(BigInt((int) std::ceil(rectf.bottomRight().x())), BigInt((int) std::ceil(rectf.topLeft().y())));
